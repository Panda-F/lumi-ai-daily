...
class _DocxBuilder:
    ...
    def add_page_break(self) -> None:
        paragraph = self._paragraph(after=0, line=240)
        run = etree.SubElement(paragraph, _qn("w", "r"))
        etree.SubElement(run, _qn("w", "br"), {_w_attr("type"): "page"})
        self.body.append(paragraph)
...
def _core_properties_xml(title: str) -> bytes:
    ...
    etree.SubElement(root, f"{{{DC_NS}}}creator").text = "Lumi AI Daily"
    etree.SubElement(root, f"{{{CP_NS}}}lastModifiedBy").text = "Lumi AI Daily"
...
def _app_properties_xml() -> bytes:
    ...
    etree.SubElement(root, f"{{{APP_NS}}}Application").text = "Lumi AI Daily"
    etree.SubElement(root, f"{{{APP_NS}}}DocSecurity").text = "0"
    etree.SubElement(root, f"{{{APP_NS}}}ScaleCrop").text = "false"
    etree.SubElement(root, f"{{{APP_NS}}}Company").text = "Lumi AI Daily"
...
def build_wechat_docx(article: Mapping[str, Any], output_path: str | Path) -> Path:
    ...
    show_title = bool(article.get("show_title"))
