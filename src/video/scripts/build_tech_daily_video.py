...
from tech_daily_publication_contract import compact_label, load_emoji_icon_map, require_mapped_emoji, sanitize_display_title
from ai_daily_llm_content import load_content_manifest
from video_build_review import review_video_script
from video_build_script import build_video_script_payload
from video_style_review import review_alignment_consistency, review_style_consistency
...
FETCH_IMAGES_SCRIPT = WORKSPACE_DIR / "skills" / "plus-media-factory" / "scripts" / "fetch_story_images.py"
...
DAILY_REPORTS_ROOT = Path("/Users/dystopia/Desktop/AI-Daily-Reports")
HTML_BASELINE = "light"
FRIENDLY_OUTPUT_SIZE_MB = 20.0
OFFICIAL_ISSUE_START_DATE = os.environ.get("AI_DAILY_OFFICIAL_START_DATE", "2026-04-13").strip()
FETCH_IMAGES_REPORT_TIMEOUT = 120
FETCH_IMAGES_URL_TIMEOUT = 45
_SKIP_LIVE_SEARCH_FALLBACK_REQUESTED = os.environ.get("AI_DAILY_SKIP_LIVE_SEARCH_FALLBACK", "0").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
}
_SKIP_LIVE_SEARCH_FALLBACK_AUTHORIZED = os.environ.get("AI_DAILY_ALLOW_LIVE_SEARCH_SKIP", "0").strip().lower() in {
    "1",
    "true",
    "yes",
    "on",
}
SKIP_LIVE_SEARCH_FALLBACK = _SKIP_LIVE_SEARCH_FALLBACK_REQUESTED and _SKIP_LIVE_SEARCH_FALLBACK_AUTHORIZED
...
_DDGS_MODULE_CACHE: Any | None = None
...
PUBLIC_TITLE_REWRITE_PATTERNS: list[tuple[re.Pattern[str], str]] = []
SPOKEN_REPLACEMENTS: list[tuple[re.Pattern[str], str]] = []
PUBLIC_SPOKEN_REPLACEMENTS: list[tuple[re.Pattern[str], str]] = []
PUBLIC_TREND_REPLACEMENTS: list[tuple[re.Pattern[str], str]] = []
...
_ASSETS_DIR = Path("/Users/dystopia/.openclaw/workspace/assets")
DEFAULT_LUMI_INTRO_IMAGE = (
    _ASSETS_DIR / "lumi-dance.gif"
    if (_ASSETS_DIR / "lumi-dance.gif").exists()
    else _latest_daily_report_asset("lumi-dance.gif") or Path("/Users/dystopia/Downloads/20190302105002_W3EzC.jpeg")
)
...
def ensure_duckduckgo_search_module() -> Any | None:
    global _DDGS_MODULE_CACHE
    if _DDGS_MODULE_CACHE is not None:
        return _DDGS_MODULE_CACHE
    spec = importlib.util.find_spec("duckduckgo_search")
    if spec is None:
        print("[search] duckduckgo-search not found, attempting install...", file=sys.stderr)
        if not install_python_package("duckduckgo-search"):
            print("[search] install failed, visual search fallback unavailable.", file=sys.stderr)
            return None
    _DDGS_MODULE_CACHE = importlib.import_module("duckduckgo_search")
    return _DDGS_MODULE_CACHE
...
def fetch_manifest(report: Path, out_dir: Path, limit: int) -> Path | None:
    ensure_dir(out_dir)
    cmd = [
        "python3",
        str(FETCH_IMAGES_SCRIPT),
        "--report",
        str(report),
        "--out-dir",
        str(out_dir),
    ...
    try:
        completed = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=FETCH_IMAGES_REPORT_TIMEOUT,
        )
    except subprocess.TimeoutExpired:
        print(
            f"[images] base manifest fetch timed out after {FETCH_IMAGES_REPORT_TIMEOUT}s for {report.name}; continuing without it.",
            file=sys.stderr,
        )
        return None
    if completed.returncode != 0:
        print(
            f"[images] base manifest fetch failed for {report.name}: {(completed.stderr or '').strip()[-400:]}",
            file=sys.stderr,
        )
        return None
...
def fetch_manifest_for_urls(urls: list[str], out_dir: Path, *, limit: int = 6, per_url_limit: int = 3) -> Path | None:
    deduped_urls = [url for url in unique(urls) if url][:limit]
    if not deduped_urls:
        return None
    ensure_dir(out_dir)
    cmd = [
    ...
    try:
        completed = subprocess.run(
            ...
            text=True,
            check=False,
            timeout=FETCH_IMAGES_URL_TIMEOUT,
        )
    except subprocess.TimeoutExpired:
        print(
            f"[images] url fetch timed out after {FETCH_IMAGES_URL_TIMEOUT}s for {len(deduped_urls)} fallback urls; continuing without fetched pages.",
            file=sys.stderr,
        )
        return None
    ...
    if completed.returncode != 0:
        print(
            f"[images] url fetch failed for fallback pages: {(completed.stderr or '').strip()[-400:]}",
            file=sys.stderr,
        )
...
def search_brand_hint(item: TechDailyItem) -> str:
    domain = source_domain(item.source_url).lower()
    if domain in {"x.com", "twitter.com", "t.co"}:
        preferred = [
            candidate
            for candidate in sorted(preferred_media_domains(item))
            if candidate and candidate not in {"x.com", "twitter.com", "t.co"}
        ]
        if preferred:
            domain = preferred[0]
    brand_map = {
    ...
    for suffix, label in brand_map.items():
...
def build_visual_search_queries(item: TechDailyItem) -> list[str]:
    title = display_title_text(item.title)
    title = re.sub(r"[“”\"'`]+", " ", title)
    title = re.sub(r"\s+", " ", title).strip()
    if not title:
        title = spoken_title_summary(item.title)
    brand = search_brand_hint(item)
    english_terms = [
        term
        for term in unique(re.findall(r"[A-Za-z][A-Za-z0-9.+-]{2,}", f"{item.title} {item.content} {item.quote or ''}"))
        if term.lower() not in {"openai", "anthropic", "meta", "hugging", "face", "engineering", "report", "preview"}
    ]
    topic_hint = " ".join(english_terms[:2]).strip()
    quote_hint = " ".join(re.findall(r"[A-Za-z]{3,}", item.quote or "")[:5]).strip()
    queries = [
        f"{title} {brand}".strip(),
        f"{title} representative image".strip(),
        f"{brand} {title} screenshot".strip(),
        f"{brand} {topic_hint} press image".strip(),
        f"{brand} {topic_hint} illustration".strip(),
    ]
    if quote_hint and brand:
        queries.append(f"{brand} {quote_hint}".strip())
    if brand and topic_hint:
        queries.append(f"{brand} {topic_hint}".strip())
    source_site = source_domain(item.source_url)
    if source_site and source_site not in {"x.com", "twitter.com", "t.co"}:
        queries.append(f"{title} site:{source_site}".strip())
    for preferred_domain in sorted(preferred_media_domains(item)):
        if preferred_domain and preferred_domain not in {"x.com", "twitter.com", "t.co"}:
            queries.append(f"{title} site:{preferred_domain}".strip())
    return [query for query in unique(queries) if query]
...
def clean_search_result_title(raw: str) -> str:
    text = re.sub(r"<[^>]+>", " ", raw or "")
    return re.sub(r"\s+", " ", html.unescape(text)).strip()


def search_result_relevance(item: TechDailyItem, *, url: str, title: str) -> int:
    host = source_domain(url).lower()
    blob = normalize_search_text(f"{title} {url}")
    term_hits, phrase_hits = topic_overlap_score(item, blob)
    specific_term_hits, specific_phrase_hits = topic_specific_overlap_score(item, blob)
    score = term_hits * 3 + phrase_hits * 6
    if media_domain_matches(item, host):
        score += 8
    if item.source_url and host == source_domain(item.source_url).lower():
        score += 4
    if media_domain_matches(item, host) and specific_term_hits == 0 and specific_phrase_hits == 0:
        score -= 7
    if is_rootish_url(url) and specific_term_hits == 0 and specific_phrase_hits == 0:
        score -= 12
    if "site:" in blob:
        score -= 2
    return score
...
def extract_search_result_entries(html_text: str, *, engine: str) -> list[dict[str, str]]:
    patterns = {
        "duckduckgo": r'result__a[^>]+href="([^"]+)"[^>]*>(.*?)</a>',
        "bing": r'<li class="b_algo".*?<a href="([^"]+)"[^>]*>(.*?)</a>',
    }
    pattern = patterns[engine]
    ...
    for url, title_html in matches:
        ...
        if (
            not normalized
            or not host
            or host in SEARCH_HOST_BLOCKLIST
            or any(host.endswith(f".{blocked}") for blocked in SEARCH_HOST_BLOCKLIST)
        ):
            continue
        entries.append({"url": normalized, "title": clean_search_result_title(title_html)})
    return entries


def search_result_entries_for_item(item: TechDailyItem, *, max_results: int = 6) -> list[dict[str, str]]:
    entries_by_url: dict[str, dict[str, str]] = {}
    preferred_results: list[dict[str, str]] = []
    prefer_official_only = source_domain(item.source_url).lower() in {"x.com", "twitter.com", "t.co"}
    for query in build_visual_search_queries(item):
        search_targets = [
        ...
        for engine, search_url in search_targets:
            try:
                entries = extract_search_result_entries(fetch_search_html(search_url), engine=engine)
            except Exception as exc:  # noqa: BLE001
                print(f"[search] {engine} search failed for item {item.index}: {exc}", file=sys.stderr)
                continue
            ...
            ranked = sorted(
                ...
                key=lambda entry: search_result_relevance(item, url=entry["url"], title=entry["title"]),
                reverse=True,
            )
            filtered = [
                entry
                for entry in ranked
                if search_result_relevance(item, url=entry["url"], title=entry["title"]) >= 4
            ]
            preferred = [
                entry
                for entry in filtered
                if media_domain_matches(item, source_domain(entry["url"]).lower())
            ]
            if preferred:
                preferred_results = preferred[:max_results]
            if len(filtered) >= max_results:
                if preferred and prefer_official_only:
                    return preferred[:max_results]
                if preferred:
                    remainder = [entry for entry in filtered if entry not in preferred]
                    return (preferred + remainder)[:max_results]
    ...
    filtered = [entry for entry in ranked if search_result_relevance(item, url=entry["url"], title=entry["title"]) >= 4]
    preferred = [
        entry
        for entry in filtered
        if media_domain_matches(item, source_domain(entry["url"]).lower())
...
def extract_search_result_urls(html_text: str, *, engine: str) -> list[str]:
    return [entry["url"] for entry in extract_search_result_entries(html_text, engine=engine)]


def search_result_urls_for_item(item: TechDailyItem, *, max_results: int = 6) -> list[str]:
    return [entry["url"] for entry in search_result_entries_for_item(item, max_results=max_results)]


def search_image_assets_for_item(item: TechDailyItem, out_dir: Path, *, max_results: int = 8) -> list[dict[str, object]]:
    entries = search_result_entries_for_item(item, max_results=max_results + 2)
    url_title_map = {entry["url"]: entry["title"] for entry in entries}
    manifest = fetch_manifest_for_urls(
        [entry["url"] for entry in entries],
        out_dir,
        limit=max_results + 2,
        per_url_limit=3,
    )
    return attach_search_metadata(load_assets(manifest), url_title_map)


def attach_search_metadata(assets: list[dict[str, object]], url_title_map: dict[str, str]) -> list[dict[str, object]]:
    enriched: list[dict[str, object]] = []
    for asset in assets:
        entry = dict(asset)
        page_url = str(entry.get("page_final_url") or entry.get("source_page") or "")
        if page_url and page_url in url_title_map:
            entry["search_title"] = url_title_map[page_url]
        selector = str(entry.get("selector") or "")
        if selector and not selector.startswith("search-"):
            entry["selector"] = f"search-{selector}"
        enriched.append(entry)
    return enriched


def extract_direct_image_urls(page_url: str, *, max_urls: int = 48) -> list[str]:
    try:
        html_text = fetch_search_html(page_url)
    except Exception:  # noqa: BLE001
        return []
    matches = re.findall(r"https?://[^\"'()\s<>]+?(?:png|jpg|jpeg|webp|gif)", html_text, flags=re.IGNORECASE)
    urls: list[str] = []
    seen: set[str] = set()
    for raw in matches:
        candidate = html.unescape(raw.strip())
        host = source_domain(candidate).lower()
        if (
            not candidate
            or not host
            or host in SEARCH_HOST_BLOCKLIST
...
def search_direct_image_assets_for_entries(
    item: TechDailyItem,
    entries: list[dict[str, str]],
    out_dir: Path,
    *,
    ...
    ensure_dir(out_dir)
    ranked_candidates: list[tuple[tuple[int, int, int, int], dict[str, str], str]] = []
    seen_urls: set[str] = set()
    for entry in entries[:4]:
        for image_url in extract_direct_image_urls(entry["url"], max_urls=48):
            if image_url in seen_urls:
                continue
            seen_urls.add(image_url)
            host = source_domain(image_url).lower()
            term_hits, phrase_hits = topic_overlap_score(item, image_url)
            specific_term_hits, specific_phrase_hits = topic_specific_overlap_score(item, image_url)
            if (
                specific_term_hits == 0
            ...
            ranked_candidates.append(
                (
                    (
                        1 if media_domain_matches(item, host) else 0,
                        specific_phrase_hits,
                        specific_term_hits,
    ...
    for _, entry, image_url in ranked_candidates:
        host = source_domain(image_url).lower()
        image_name = Path(urllib.parse.urlparse(image_url).path).name
        identity = f"{host}:{normalized_media_identity(image_name)}"
        if not image_name or not identity or identity in seen_keys:
            continue
        seen_keys.add(identity)
        suffix = Path(image_name).suffix or ".img"
        safe_stem = re.sub(r"[^a-zA-Z0-9._-]+", "-", Path(image_name).stem).strip("-") or "search-image"
        destination = out_dir / f"{len(assets) + 1:02d}-{host}-{safe_stem[:72]}{suffix}"
        try:
        ...
        assets.append(
            {
                ...
                "kind": "image",
                "selector": "search-direct",
                "image_url": image_url,
                "source_page": entry["url"],
                "page_final_url": entry["url"],
...
def reuse_previous_build_search_assets(item: TechDailyItem, out_dir: Path) -> list[dict[str, object]]:
    try:
    ...
    if not day_root.exists():
        return []
    assets: list[dict[str, object]] = []
    item_dir_name = f"item-{item.index:02d}"
    previous_roots = [
        candidate
    ...
    for previous_root in previous_roots:
        manifests = [
            previous_root / "assets" / "search-fallback" / item_dir_name / "official-pages" / "manifest.json",
            previous_root / "assets" / "search-fallback" / item_dir_name / "pages" / "manifest.json",
            previous_root / "assets" / "search-fallback" / item_dir_name / "images" / "manifest.json",
        ]
        for manifest in manifests:
            assets.extend(load_assets(manifest))
        if assets:
            return assets
    return assets
...
def reuse_previous_build_selected_media(item: TechDailyItem, out_dir: Path) -> list[dict[str, object]]:
    ...
    for previous_root in previous_roots:
        ...
        for scene in scenes:
            ...
            search_manifests = [
                previous_root / "assets" / "search-fallback" / f"item-{item.index:02d}" / "official-pages" / "manifest.json",
                previous_root / "assets" / "search-fallback" / f"item-{item.index:02d}" / "pages" / "manifest.json",
                previous_root / "assets" / "search-fallback" / f"item-{item.index:02d}" / "images" / "manifest.json",
            ]
            for candidate_manifest in search_manifests:
                cached_candidates.extend(load_assets(candidate_manifest))
...
def search_fallback_assets(item: TechDailyItem, out_dir: Path, *, min_assets: int = 2) -> list[dict[str, object]]:
    ensure_dir(out_dir)
    def dedupe(collected: list[dict[str, object]]) -> list[dict[str, object]]:
        deduped: list[dict[str, object]] = []
        seen_keys: set[str] = set()
        for asset in collected:
            file_path = str(asset.get("file") or "")
            identity = media_identity_key(asset)
    ...
    assets: list[dict[str, object]] = []
    official_urls = [url for url in official_story_url_candidates(item, limit=12) if url_exists(url)]
    if official_urls:
        official_entries = [{"url": url, "title": url} for url in official_urls]
        official_manifest = fetch_manifest_for_urls(
            official_urls,
            out_dir / "official-pages",
            limit=min(8, len(official_urls)),
            per_url_limit=3,
        )
        assets.extend(load_assets(official_manifest))
        assets.extend(search_direct_image_assets_for_entries(item, official_entries, out_dir / "official-direct", max_results=12))
    if len(assets) >= min_assets:
        return dedupe(assets)
    assets.extend(reuse_previous_build_selected_media(item, out_dir))
    if len(assets) >= min_assets:
        return dedupe(assets)
    if SKIP_LIVE_SEARCH_FALLBACK:
        print(
            f"[images] skipping live search fallback for item {item.index}; using only explicitly collected assets.",
            file=sys.stderr,
        )
        return dedupe(assets)

    entries = search_result_entries_for_item(item, max_results=10)
    url_title_map = {entry["url"]: entry["title"] for entry in entries}
    page_manifest = fetch_manifest_for_urls(
        [entry["url"] for entry in entries],
        out_dir / "pages",
        limit=10,
        per_url_limit=3,
    )
    assets.extend(attach_search_metadata(load_assets(page_manifest), url_title_map))
    assets.extend(search_direct_image_assets_for_entries(item, entries, out_dir / "direct", max_results=12))
    if len(assets) < min_assets:
        assets.extend(search_image_assets_for_item(item, out_dir / "images", max_results=12))
    if len(assets) < min_assets:
        assets.extend(reuse_previous_build_search_assets(item, out_dir))
    return dedupe(assets)
    ...
    *,
    ...
    for asset in image_assets:
        ...
        if selector.startswith("search"):
            score += 8.0
            reasons.append("search-fallback")
        if selector == "meta":
            score -= 4.0
...
def display_title_text(text: str) -> str:
    return sanitize_display_title(text)
...
def _normalize_sentence_pairs(raw: dict[str, Any], *, context: str) -> list[dict[str, Any]]:
    ...
    for offset, pair in enumerate(pairs, start=1):
        ...
        oral = normalize_spoken_text(str(pair.get("oral") or ""))
        subtitle = normalize_spoken_text(str(pair.get("subtitle") or ""))
...
def _normalize_screen_cards(raw: dict[str, Any], item: TechDailyItem) -> list[dict[str, str]]:
    ...
    for offset, card in enumerate(cards, start=1):
        ...
        if not heading or not body or not icon_hint:
            raise RuntimeError(f"screen_cards[{offset}] for item {item.index} requires heading, body and icon_hint")
        normalized.append({"heading": heading, "body": body, "icon_hint": icon_hint})
    if len(normalized) != 3:
...
def issue_label_for_report(report: TechDailyReport, item_count: int) -> str:
    ...
    desktop_root = Path("/Users/dystopia/Desktop/AI-Daily-Reports")
    if desktop_root.exists():
        candidates.extend(desktop_root.glob("20??-??-??/tech-daily-20??-??-??.md"))
    workspace_reports = WORKSPACE_DIR / "reports"
    if workspace_reports.exists():
        candidates.extend(workspace_reports.glob("tech-daily-20??-??-??.md"))

    unique_candidates = {candidate.name: candidate for candidate in candidates}.values()
    ...
    ordered = sorted(unique_candidates, key=lambda item: item.name)
    for index, candidate in enumerate(ordered, start=1):
        if candidate.name == f"tech-daily-{report.date}.md":
            return f"第 {index} 期"
    return f"今日 {item_count} 条"
...
def build_scene_manifest(
    *,
    ...
    for offset, item in enumerate(selected_items, start=1):
        ...
        item_labels.append(nav_label or compact_label(item.title, limit=8))
    opening_items = [
        {
            "index": offset,
            "label": sanitize_display_title(item.title),
    ...
    intro_scene = {
        ...
        "trend_words": [audience_friendly_trend_word(term) for term in (report.trend_words[:3] or report.trend_lines[:3])],
        "headlines": [sanitize_display_title(item.title) for item in selected_items],
        ...
        "lead_title": sanitize_display_title(selected_items[0].title) if selected_items else "今日 AI 速递",
    ...
    for offset, item in enumerate(selected_items, start=1):
        ...
        scene = {
            ...
            "short_title": sanitize_display_title(oral["display_title"] or display_title_text(item.title)),
            "display_icon": require_mapped_emoji(item.title, emoji_map),
            "content": shorten_text(item.content, 180),
            "interpretation": shorten_text(item.interpretation, 130),
            "quote": shorten_text(item.quote or item.title, 68),
            "hook": oral["hook"],
    ...
    manifest = {
        ...
        "meta": {
            ...
            "issue_quote_text": str(outro_oral.get("quote_translation") or outro_oral.get("quote_text") or ""),
            "issue_quote_original": str(outro_oral.get("quote_text") or ""),
            "issue_quote_author": str(outro_oral.get("quote_author") or ""),
            "quote_id": str(outro_oral.get("quote_id") or ""),
        },
        "report": {
            "trend_words": [audience_friendly_trend_word(term) for term in report.trend_words[:3]],
            "items": [
                {
                    "index": item.index,
...
def still_specs_for_manifest(remotion_manifest: dict[str, Any], slides_dir: Path) -> list[dict[str, Any]]:
    ...
    still_specs = [
        {
            "output": str(slides_dir / "cover.png"),
...
def stills_only_image_spec(item: TechDailyItem) -> dict[str, Any]:
    media_assets: list[dict[str, Any]] = []
    return {
        "image_paths": [],
        "public_image_src": None,
        "public_image_srcs": [],
        "media_assets": media_assets,
        "primary_media_src": None,
        "primary_media_kind": None,
        "media_usage": "fallback_card",
        "layout_variant": detect_layout_variant(item, media_assets),
        "template_variant": detect_template_variant(item, media_assets),
        "card_type": detect_card_type(item, media_assets),
        "image_reviews": [],
        "approved_image_count": 0,
        "truthful_visual_count": 0,
        "scene_visual_count": 0,
        "visual_coverage_status": "preview",
        "media_alignment_review": {
            "status": "pass",
            "summary": "stills-only preview uses latest editorial screen copy without regenerating TTS or MP4",
        },
        "media_reject_reason": "",
    }
...
def main() -> int:
    ...
    content_manifest_arg = args.content_manifest or args.editorial_bundle
    ...
    review_path: Path | None = out_dir / "video-review.json"
    ...
    cover_path: Path | None = out_dir / "slides" / "cover.png"
    ...
    try:
        ...
        current_stage = "machine_review"
        review_payload = review_video_script(report, selected_items, video_script_payload)
        video_script_path = write_json(out_dir / "video-script.json", video_script_payload)
        review_path = write_json(out_dir / "video-review.json", review_payload)
        existing_machine_review = report_sidecar.get("machine_review") if isinstance(report_sidecar, dict) else {}
        report.machine_review = {
            **(existing_machine_review if isinstance(existing_machine_review, dict) else {}),
            **review_payload,
        }
        report_json_path = write_report_json(report, report_json_path_for_markdown(report.path))
        if review_payload["status"] != "pass":
            raise RuntimeError(f"Machine review failed: {review_payload.get('blocking_findings', [])}")
        ...
        manifest_path = Path(args.manifest).expanduser().resolve() if args.manifest else fetch_manifest(Path(report.path), assets_dir, args.max_items)
        ...
        if args.stills_only:
            ...
            image_specs = [stills_only_image_spec(item) for item in selected_items]
            for offset, item in enumerate(selected_items, start=1):
            ...
            video_script_payload["segments"][0]["trend_words"] = [
                audience_friendly_trend_word(term) for term in (report.trend_words[:3] or report.trend_lines[:3])
            ]
            ...
            style_review = review_style_consistency(
                report_path=Path(report.path),
                out_dir=out_dir,
                slides_dir=slides_dir,
                html_baseline=HTML_BASELINE,
            )
            remotion_manifest["meta"]["style_review_status"] = style_review.get("status", "unknown")
            remotion_manifest_path = write_json(out_dir / "remotion-manifest.json", remotion_manifest)
            ...
            summary = {
                ...
                "video_review": str(review_path),
                "machine_review": review_payload,
                "cover_image": str(slides_dir / "cover.png"),
                "video_cover_image": str(slides_dir / "cover.png"),
                "video": None,
                "srt": None,
                "segments": summary_entries,
        ...
        for offset, item in enumerate(selected_items, start=1):
            ...
            local_reviewed_assets = merge_media_candidates(
                approved_assets,
                support_assets,
                max_images=2,
            )

            search_review: dict[str, Any] | None = None
            if len(local_reviewed_assets) < args.min_reviewed_images:
                fallback_assets = search_fallback_assets(item, assets_dir / "search-fallback" / f"item-{offset:02d}", min_assets=2)
                if fallback_assets:
                    search_review = audit_image_candidates(item, fallback_assets, max_images=4)

            reviewed_assets = merge_media_candidates(
                approved_assets,
                list((search_review or {}).get("approved_assets") or []),
                support_assets,
                list((search_review or {}).get("support_assets") or []),
                max_images=2,
            )
            if len(reviewed_assets) < args.min_reviewed_images:
                broad_fallback = search_fallback_assets(
                    item, assets_dir / "search-fallback-broad" / f"item-{offset:02d}", min_assets=4
                )
                if broad_fallback:
                    broad_review = audit_image_candidates(item, broad_fallback, max_images=6)
                    reviewed_assets = merge_media_candidates(
                        reviewed_assets,
                        list(broad_review.get("approved_assets") or []),
                        list(broad_review.get("support_assets") or []),
                        max_images=2,
                    )
            if len(reviewed_assets) < args.min_reviewed_images:
                import logging
                ...
                logging.warning(
                    f"Item {offset} only has {len(reviewed_assets)} reviewed images after audit, below required {args.min_reviewed_images}."
                )
            media_alignment_review = build_media_alignment_review(
                item,
                item_script,
                reviewed_assets,
                min_required=args.min_reviewed_images,
            )
            ...
            image_specs.append(
                {
                    ...
                    "public_image_src": public_images[0] if public_images else None,
                    "public_image_srcs": public_images,
                    "media_assets": public_media_assets,
                    "primary_media_src": public_media_assets[0]["src"] if public_media_assets else None,
                    "primary_media_kind": public_media_assets[0]["kind"] if public_media_assets else None,
                    "media_usage": public_media_assets[0]["kind"] if public_media_assets else "fallback_card",
                    "layout_variant": layout_variant,
                    "template_variant": template_variant,
                    "card_type": detect_card_type(item, public_media_assets),
                    "image_reviews": media_review["reviewed_assets"] + list((search_review or {}).get("reviewed_assets") or []),
                    ...
                    "visual_coverage_status": "pass" if len(reviewed_assets) >= 1 and len(public_media_assets) >= args.min_reviewed_images else "fail",
                    ...
                    "media_reject_reason": combine_reject_reasons(
                        ...
                        (search_review or {}).get("media_reject_reason"),
                        None if len(reviewed_assets) >= 2 else "search-fallback-insufficient",
                    ),
                }
            )
        ...
        if public_lumi_intro and public_lumi_intro_kind != "video":
        ...
        cover_path = slides_dir / "cover.png"
        still_specs = still_specs_for_manifest(remotion_manifest, slides_dir)
        ...
        alignment_review = review_alignment_consistency(summary_entries, fps=FPS)
        style_review = review_style_consistency(
            report_path=Path(report.path),
            out_dir=out_dir,
            slides_dir=slides_dir,
        ...
        if output_size_mb > FRIENDLY_OUTPUT_SIZE_MB:
            style_review.setdefault("warnings", []).append(
        ...
        remotion_manifest["meta"]["live_search_fallback_skipped"] = SKIP_LIVE_SEARCH_FALLBACK
        remotion_manifest_path = write_json(out_dir / "remotion-manifest.json", remotion_manifest)
        tts_summary = summarize_tts_usage([*audio_specs, outro_audio_spec], tts_state)
        ...
        summary = {
            ...
            "live_search_fallback_skipped": SKIP_LIVE_SEARCH_FALLBACK,
            "render": render_result,
        ...
        if report_root.exists():
            ...
            for stale_name in ("gemini-help.txt",):
                stale = report_root / stale_name
                if stale.exists():
                    stale.unlink(missing_ok=True)
        print(json.dumps(summary, ensure_ascii=False, indent=2))
        return 0
        ...
        if review_payload:
            failure_summary["machine_review"] = review_payload
