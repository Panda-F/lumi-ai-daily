...
def x_reader_path() -> Path:
    return Path(__file__).resolve().parent / "x_reader.py"
...
def archive_wechat(session: requests.Session, url: str, item_dir: Path) -> ArchiveResult:
    return archive_html_like(session, url, item_dir, "wechat", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))


def archive_zhihu(session: requests.Session, url: str, item_dir: Path) -> ArchiveResult:
    return archive_html_like(session, url, item_dir, "zhihu", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))


def archive_xiaohongshu(session: requests.Session, url: str, item_dir: Path) -> ArchiveResult:
    return archive_html_like(session, url, item_dir, "xiaohongshu", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))


def archive_xueqiu(session: requests.Session, url: str, item_dir: Path) -> ArchiveResult:
    return archive_html_like(session, url, item_dir, "xueqiu", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))
...
def browser_cli_binary() -> str | None:
    return None


def run_browser_cli(browser_profile: str, args: list[str], timeout: int = 45) -> subprocess.CompletedProcess[str]:
    binary = browser_cli_binary()
    if not binary:
        raise FileNotFoundError("browser CLI fallback is disabled in the AI Daily mainline")
...
def browser_capture_page(url: str, kind: str, item_dir: Path, browser_profile: str) -> BrowserCapture | None:
    ...
    try:
        opened = run_browser_cli(browser_profile, ["open", url], timeout=30)
        ...
        run_browser_cli(browser_profile, ["wait", "--target-id", target_id, "--time", "2500"], timeout=10)
...
"""
        ...
        if target_id:
            try:
                run_browser_cli(browser_profile, ["close", target_id], timeout=10)
...
def archive_sources(
    ...
    for idx, url in enumerate(urls, start=1):
        ...
        if kind == "x":
            ...
            result = archive_html_like(session, url, item_dir, "wechat", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))
        elif kind == "zhihu":
            result = archive_html_like(session, url, item_dir, "zhihu", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))
        elif kind == "xiaohongshu":
            result = archive_html_like(session, url, item_dir, "xiaohongshu", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))
        elif kind == "xueqiu":
            result = archive_html_like(session, url, item_dir, "xueqiu", os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"))
        else:
            continue
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--browser-profile",
        default=os.environ.get("AI_DAILY_BROWSER_PROFILE", "disabled"),
        help="Browser profile label for optional browser fallback. The mainline keeps this disabled.",
    )
    parser.add_argument(
        "--max-per-kind",
        type=int,
...
def main() -> int:
    args = parse_args()
    os.environ["AI_DAILY_BROWSER_PROFILE"] = args.browser_profile
