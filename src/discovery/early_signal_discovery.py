...
class LinkExtractor(HTMLParser):
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        if tag == "a" and self._current_href:
            self.links.append({"href": self._current_href, "text": normalize_spaces(" ".join(self._current_text))})
            self._current_href = ""
            self._current_text = []
...
def extract_links(raw_html: str) -> LinkExtractor:
    ...
    *,
    ...
    for monitor in monitors:
        ...
        try:
            ...
            continue
        page = extract_links(raw_html)
        page_title = page.title or company
        ...
        for anchor in page.links:
            ...
            if source_host(href) != source_host(url):
                continue
            label = normalize_spaces(anchor.get("text", ""))
...
def discover_hf_papers(session: requests.Session, url: str, *, timeout: int, max_items: int) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    for anchor in page.links:
        ...
        if not href.startswith("/papers/"):
            continue
        title = normalize_spaces(anchor.get("text", ""))
