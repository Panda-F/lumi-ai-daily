...
from ai_daily_llm_content import generate_editorial_bundle
...
from tech_daily_publication_contract import build_publication_contract

WORKSPACE_DIR = Path(__file__).resolve().parents[1]
SEARCH_TERMS_SCRIPT = WORKSPACE_DIR / "scripts" / "tech_daily_search_terms.py"
CANDIDATE_REVIEW_SCRIPT = WORKSPACE_DIR / "scripts" / "tech_daily_candidate_review.py"
REFERENCE_ARCHIVE_SCRIPT = WORKSPACE_DIR / "scripts" / "archive_reference_sources.py"
ILLUSTRATED_RENDER_SCRIPT = WORKSPACE_DIR / "scripts" / "render_illustrated_report.py"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--candidate-review-out", help="Optional output path for candidate review.")
    ...
    parser.add_argument("--illustrated-out", help="Optional explicit output path for the illustrated markdown report.")
    parser.add_argument("--image-manifest-out", help="Optional explicit output path for the illustrated image manifest.")
    ...
    parser.add_argument("--skip-candidate-review", action="store_true", help="Skip candidate-review generation.")
    ...
    parser.add_argument("--skip-illustrated-report", action="store_true", help="Skip illustrated markdown derivation.")
    parser.add_argument("--lookback-hours", type=int, default=24, help="Hot-window hours for candidate review.")
    parser.add_argument("--top-n", type=int, default=20, help="Candidate review shortlist size.")
...
def enrich_report_with_publication_refs(
    ...
    confirmation_by_index = {
        ...
        for entry in publication_contract.get("items") or []
    ...
    *,
    ...
    for item in compiled_report.items:
        ...
        if image_cards:
            ...
            lines.append("- 当前 source pack 没有可用图片；兜底时应截图来源页面的标题、核心图表、公告正文或社媒正文区域。")
        lines.extend(
            [
                "",
                "### 原始相关新闻原稿",
...
def run_illustrated_report_render(
    report_path: Path,
    illustrated_out: Path,
    image_manifest_out: Path,
    reference_pack_out: Path,
    source_pack_out: Path,
) -> tuple[dict[str, Any], str]:
    try:
        payload = run_json(
            [
                sys.executable,
                str(ILLUSTRATED_RENDER_SCRIPT),
                "--report",
                str(report_path),
                "--out",
                str(illustrated_out),
                "--manifest-out",
                str(image_manifest_out),
                "--reference-pack",
                str(reference_pack_out),
                "--source-pack",
                str(source_pack_out),
...
def main() -> int:
    ...
    candidate_review_out = Path(args.candidate_review_out).expanduser().resolve() if args.candidate_review_out else discovery_dir / "candidate-review.json"
    ...
    illustrated_out = Path(args.illustrated_out).expanduser().resolve() if args.illustrated_out else report_path.with_name(f"{report_path.stem}.illustrated.md")
    ...
    if discovery_json.exists():
        ...
        if not args.skip_candidate_review:
            cmd = [
                sys.executable,
                ...
                str(args.top_n),
                "--out",
                str(candidate_review_out),
            ]
            for source_pack_dir in source_pack_args:
                cmd.extend(["--source-pack", str(source_pack_dir)])
            candidate_review_payload = run_json(cmd)
        else:
    ...
    compiled_report = enrich_report_with_publication_refs(compiled_report, publication_contract)
    compiled_report.machine_review["publication_contract"] = publication_contract
    ...
    illustrated_payload: dict[str, Any] = {}
    illustrated_error = ""
    if not args.skip_illustrated_report:
        illustrated_payload, illustrated_error = run_illustrated_report_render(
            report_path,
            illustrated_out,
            image_manifest_out,
            reference_pack_out,
    ...
    compiled_report = enrich_report_with_publication_refs(compiled_report, publication_contract)
    if not args.skip_illustrated_report:
        refreshed_illustrated_payload, refreshed_illustrated_error = run_illustrated_report_render(
            report_path,
            illustrated_out,
            image_manifest_out,
            reference_pack_out,
        ...
        if refreshed_illustrated_payload:
            illustrated_payload = refreshed_illustrated_payload
        if refreshed_illustrated_error:
            illustrated_error = refreshed_illustrated_error
    fact_draft_out.write_text(
        build_traceable_fact_draft(
            compiled_report,
            candidate_review=candidate_review_payload,
    ...
    payload = {
        ...
        "illustrated_report": str(illustrated_out) if illustrated_out.exists() else None,
        ...
        "confirmed_item_count": int(publication_contract.get("confirmed_item_count") or 0),
        "publication_contract_status": publication_contract.get("status"),
        ...
        "candidate_review_generated": bool(candidate_review_payload),
        "reference_pack_generated": bool(reference_pack_payload),
        "reference_pack_error": reference_pack_error or None,
        "illustrated_report_generated": bool(illustrated_payload),
        "illustrated_report_error": illustrated_error or None,
    }
