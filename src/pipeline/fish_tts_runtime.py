...
DEFAULT_ENDPOINT = "http://192.168.1.13:8888/v1/tts"
...
def _run_internal_agent(
    *,
    ...
    payload = _parse_json_blob(completed.stdout)
    if payload.get("status") != "ok":
        raise FishTtsError(f"Internal agent returned non-ok status: {payload}")
    return payload
...
def convert_to_voice_note(source_path: pathlib.Path, voice_note_path: pathlib.Path) -> None:
    ...
    if result.returncode != 0:
        raise FishTtsError(
            f"ffmpeg failed while converting to Telegram voice note: {result.stderr[-1000:]}"
        )
...
def send_telegram_voice(
    *,
    ...
    cmd = [
        ...
        f"chat_id={target}",
        "-F",
        f"voice=@{voice_note_path};type=audio/ogg",
    ]
    if caption:
    ...
    if result.returncode != 0:
        raise FishTtsError(f"curl failed while sending the Telegram voice note: {result.stderr[-1000:]}")
    try:
        payload = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise FishTtsError(f"Telegram returned non-JSON output: {result.stdout[-1000:]}") from exc
    if not payload.get("ok"):
        raise FishTtsError(f"Telegram sendVoice failed: {payload}")
    return payload
