...
from ai_daily_paths import extract_tech_daily_date, tech_daily_final_report_json_path, tech_daily_report_json_path
...
def report_json_path_for_markdown(report_path: str | Path) -> Path:
    ...
    if report_date:
        if path.name == "report.md" and path.parent.name == "final":
            return tech_daily_final_report_json_path(report_date)
